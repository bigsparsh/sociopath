...
const UploadPost = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get(import.meta.env.VITE_BACKEND_URL + "/user/me?jwt=" + localStorage.getItem("auth-token")).then((res) => {
  ...
  const uploadPost = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await axios.post(import.meta.env.VITE_BACKEND_URL + "/post/create", {
    ...
    <div className="flex justify-evenly gap-5 flex-col lg:flex-row items-center p-4 lg:p-10 h-full overflow-x-clip">
        ...
        <div className="card shrink-0 w-full lg:max-w-sm shadow-2xl bg-base-300/70">
          <form className="card-body" encType="multipart/form-data" onSubmit={uploadPost}>
            <div className="form-control">
              ...
              <textarea className="textarea textarea-bordered placeholder:text-xs bg-base-100/50 backdrop-blur-sm" onChange={(e) => {
            ...
            <div className="form-control">
              ...
              }} accept="image/*" className="file-input-sm w-full placeholder:text-xs bg-base-100/50 backdrop-blur-sm file-input file-input-bordered w-full lg:max-w-xs" />
            </div>
            <div className="form-control">
              <label className="label">
              ...
              <div className="w-full flex justify-start text-xs items-center gap-3">
                <input type="checkbox" onChange={() => { setEnableComment(e => !e) }} className="toggle" />
              </div>
            </div>
      ...
      <div className="lg:basis-2/3 w-full">
        <div className={postImage == "NO IMAGE" ? `flex x-20 border border-base-100 flex-col opacity-90 bg-base-300 rounded-xl backdrop-blur-sm shadow-xl max-h-[600px] z-50` : `flex flex-col bg-base-300 border border-base-100 rounded-xl shadow-xl relative h-[600px] z-10`}>
                  ...
                  currentUser?.profile_image == "NO IMAGE" ?
                    <div className="avatar placeholder">
                      <div className="bg-neutral text-neutral-content rounded-full w-12">
                        <span className="text-lg">{currentUser.name.split(" ")[0][0].toUpperCase()}{currentUser.name.split(" ")[1][0].toUpperCase()}</span>
                      </div>
                    </div>
                    :
          ...
          {
            postImage == "NO IMAGE" ? null :
              <div className="bg-cover bg-center grow opacity-80" style={{ backgroundImage: `url(${postImage})` }}></div>

          }
          <div className="text-cont max-h-[200px] overflow-auto bg-base-300">
            <p className="p-5">{description}</p>
          </div>
          <p className="text-xs  px-5 pt-3 bg-base-300">Posted at {String(new Date())} </p>
          <div className="flex bg-base-300 p-5 justify-start lg:gap-16 items-center rounded-b-xl">
            ...
            <button className="btn btn-ghost flex gap-3 items-center" id="dislike" >
              <HiMiniHandThumbDown className={`text-xl pointer-events-none`} /> 64
            </button>
            {
              enableComment ?
                ...
                :
                <button className="btn btn-ghost flex gap-3 items-center" disabled>
                  <HiChatBubbleLeft className="text-xl" /> 34
                </button>
            }
          </div>
