    ...
    <>
      <div className="flex lg:flex-row flex-col-reverse lg:gap-10 gap-0">
        <div className="fixed text-transparent inset-x-24 inset-y-12 lg:inset-x-1/4 w-fit h-fit flex-col flex justify-center items-center text-7xl lg:text-9xl font-black tracking-wide z-[-1]">
          <h1 className="bg-gradient-to-r from-primary/50 via-primary to-primary/50 bg-clip-text">PROFILE</h1>
        </div>

        < div className="p-3 xl:p-10 grow lg:basis-2/3 w-full" >
        <h1 className="text-xl font-semibold lg:hidden block p-5">Your Posts </h1>
              ...
              currentUser?.post.length == 0 ?
                ...
                currentUser?.post.map((ele) => {
                  return ele.post_image == "NO IMAGE" ?
                    <div className="aspect-square rounded-lg bg-gradient-to-b text-white backdrop-blur-sm shadow-2xl hover:scale-110 duration-200 p-3 lg:p-5 overflow-hidden from-slate-700 lg:text-base to-slate-700/10">{ele.description}</div>
                    :
                    <div style={{ backgroundImage: `url('${ele.post_image}')` }} key={ele.post_id} className={`hover:scale-110 shadow-2xl duration-200 bg-center bg-cover aspect-square rounded-lg bg-base-300  `}></div>
                })
            }
          </div>
